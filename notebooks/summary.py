...
TIER1_HARMONIZED_DICT = os.path.join(ROOT_DIR, "reference/RADx-rad_tier1_dict_2025-03-19.csv")
TIER2_HARMONIZED_DICT = os.path.join(ROOT_DIR, "reference/RADx-rad_tier2_dict_2025-03-19.csv")
...
def get_basic_data(origcopy_dir, summary_dir):
    df_list =[]
    dict_files = glob.glob(os.path.join(origcopy_dir, f"rad_*_*-*_*_DICT_origcopy.csv"))
    if len(dict_files) == 0:
        return pd.DataFrame(columns=["subproject", "phs_id", "project_num", "radx_id"])
